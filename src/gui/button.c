...
void        update_button(t_button *my_button, char with_text, SDL_Renderer *renderer, char with_alph) //не трогать
{
    if (with_text)
    ...
    if (!with_alph || with_text)
    {
        if (my_button->father)
        {
            ...
            my_button->my_rect.x = my_button->x * g_win_width - (float)my_button->my_rect.w / 2;
            my_button->my_rect.y = my_button->y * g_win_height - (float)my_button->my_rect.h / 2;
        }
    }
